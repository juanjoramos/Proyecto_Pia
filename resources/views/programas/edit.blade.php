<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-white leading-tight">Editar Programa</h2>
    ...
    <div class="py-12 px-6 flex justify-center bg-gray-900">
        <form action="{{ route('programas.update', $programa) }}" method="POST"
            ...
            <h3 class="text-2xl font-bold mb-6 border-b border-gray-700 pb-2">Editar Programa</h3>

            {{-- Nombre --}}
            <div class="mb-5">
                <label class="block mb-2 font-semibold">Nombre del Programa</label>
                ...
                <select name="facultad_id"
                    ...
                    @foreach ($facultades as $facultad)
                        <option value="{{ $facultad->facultad_id }}"
                            {{ old('facultad_id', $programa->facultad_id) == $facultad->facultad_id ? 'selected' : '' }}>
                            {{ $facultad->nombre }}
                        </option>
                    @endforeach
                </select>
