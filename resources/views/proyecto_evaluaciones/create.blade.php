<x-app-layout>
    <x-slot name="header">
        <h1 class="text-lg font-medium">Crear ProyectoEvaluacion</h1>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('proyecto_evaluaciones.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="evaluador_id" id="evaluador_id" required class="mt-1 block w-full">
                            ...
                            @foreach ($evaluadores as $evaluador)
                                <option value="{{ $evaluador->id }}" {{ old('evaluador_id') == $evaluador->id ? 'selected' : '' }}>
                                    {{ $evaluador->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="resultados_criterios" id="resultados_criterios" rows="4" required class="mt-1 block w-full">{{ old('resultados_criterios') }}</textarea>
                    </div>
